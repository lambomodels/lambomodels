...
LM.renderTags = function(item, base) {
  ...
  if (item.color) {
    ...
    const textCol = hex && LM.isLightColor(hex) ? '#1a1a0a' : '#f0ece4';
    const style = hex ? `style="background:${hex}28;color:${textCol};border-color:${hex}55;"` : '';
    const dot   = hex ? `<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${hex};margin-right:5px;vertical-align:middle;border:1px solid rgba(255,255,255,0.15);"></span>` : '';
...
// ── PATH / NAV ────────────────────────────────────────────────
function getBase(){ const p=window.location.pathname; return p.substring(0,p.lastIndexOf('/')+1); }
LM.basePath=getBase;
LM.markActiveNav=function(){ const cur=window.location.pathname.split('/').pop().replace('.html','')||'index'; document.querySelectorAll('.nav-links a,.mobile-menu a').forEach(a=>{ const p=(a.getAttribute('href')||'').split('/').pop().replace('.html','')||'index'; a.classList.toggle('active',p===cur); }); };
LM.initHamburger=function(){ const b=document.querySelector('.hamburger'),m=document.querySelector('.mobile-menu'); if(!b||!m)return; b.addEventListener('click',()=>{ const o=m.classList.toggle('open'); document.body.style.overflow=o?'hidden':''; }); };
